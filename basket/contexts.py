...
# Define a custom message level for flat delivery charge
FLAT_DELIVERY_MESSAGE_LEVEL = 35
...
def basket_contents(request):
    ...
    if user.is_authenticated:
        ...
        if hasattr(user, 'userprofile') and hasattr(user.userprofile, 'default_country'):
            ...
            print(f"User's profile country code: {country_code}")  # Print the country code to the console
    ...
    if total == 0:
        ...
        if total < settings.FREE_DELIVERY_THRESHOLD:
            ...
            if flat_fee_applied:
                messages.add_message(request, FLAT_DELIVERY_MESSAGE_LEVEL, "Flat Delivery Charge is applied because we have no normal delivery to your country.")
        else:
            delivery = 0
            free_delivery_delta = 0
